...
const AIOutfitmaker = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/smart-search?query=${encodeURIComponent(searchQuery)}`
      );
  ...
  return (
    <SafeAreaView className="bg-white">
      <View className="relative w-full h-12 flex-row items-center px-4">
        {/* Left button */}
        ...
        {/* Centered title */}
        <Text className="flex-1 text-center text-lg font-semibold">Outfit</Text>
